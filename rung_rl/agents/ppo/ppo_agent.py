...
class PPOAgent:
    ...
    def __init__(self, train=True) -> None:
        ...
        self.name = "ppo"
        self.actor_critic = PPONetwork(INPUTS, HIDDEN_SIZE, ACTOR_SIZE, CRITIC_SIZE, NUM_ACTIONS).to(device)
        # self.actor_critic.share_memory()
        self.optimizer = optim.Adam(self.actor_critic.parameters(), lr=LR)
        self.ppo = PPO(self.actor_critic, self.optimizer)
    ...
    def get_player(self, train=True):
        ...
        """
        player = PPOPlayer(self.actor_critic, train)
    ...
    def gather_experience(self):
        ...
        for player in self.players:
            self.state_batch += player.states
            self.action_batch += player.actions
            self.log_probs_batch += player.log_probs
            self.reward_batch += player.returns
    ...
    def save_model(self, i="final"):
        torch.save(self.actor_critic.state_dict(), self.model_path(self.name, i))
    ...
    def load_model(self, i="final"):
        try:
            state_dict = torch.load(self.model_path(self.name))
...
class PPOPlayer:
    def __init__(self, actor_critic, train=True):
    ...
    def reward(self, r, player, done=False):
        ...
        self.total_reward += r
        if self.train:
            self.rewards.append(torch.tensor([r], dtype=torch.float).to(device))
            if done:
