...
class Oracle:
    ...
    def get_rung_obs(self, state):
        obs = state.get_obs()
        return torch.tensor([obs.get_oracle_rung_vector()], dtype=torch.float).to(device)

    def get_obs(self, state):
        obs = state.get_obs()
        return torch.tensor([obs.get_oracle_vector()], dtype=torch.float).to(device)

    def get_eps(self, player=0):
        """
        Returns the eps for the current step for the agent
        """
    ...
    def model_path(self, i):
        return "{}/model_{}_{}".format(MODEL_PATH, "oracle", i)
